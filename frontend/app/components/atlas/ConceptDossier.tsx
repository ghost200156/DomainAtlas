...
import type { AtlasDocument, AtlasModule, ConceptNode, DemoRun } from "../../lib/types";
import { cleanLabel, renderMarkdown, RELATION_LABELS } from "./atlasUtils";

type ConceptRelation = AtlasDocument["relations"][number];

interface ConceptDossierProps {
  selected: ConceptNode;
  selectedModule: AtlasModule | undefined;
  selectedRelations: ConceptRelation[];
  run: DemoRun;
  revealedExamples: Set<string>;
  searchLoading: boolean;
  searchResults: { title: string; url: string; snippet: string; source: string; isNew?: boolean }[];
...
export function ConceptDossier({
  ...
  const atlasIndex = {
    conceptsById: new Map(run.atlas?.concepts.map((concept) => [concept.id, concept])),
  };
          ...
          {pairs.map((pair, idx) => {
            ...
            return (
              <div key={idx} className="example-question">
                ...
                {pair.a ? (
                  <>
                  <button className="spoiler-toggle" onClick={() => onRevealToggle(qid)}>
                    {revealed ? '▲ 收起解法' : '▶ 显示解法'}
                  </button>
                  {revealed ? (
